...
public class SparkLdaOnline {
	...
	public static void main(String[] args) throws IOException {
		...
		UDF1<WrappedArray<String>, String[]> udftagger = new UDF1<WrappedArray<String>, String[]>() {
			...
			private static final long serialVersionUID = 3024033935208998950L;
		...
		for (int i = 0; i < paramjson.length(); i++) {
			...
			LDA lda = new LDA().setSeed(80).setMaxIter(iterations).setK(k).setOptimizer("online")
					.setDocConcentration(alpha).setTopicConcentration(beta).setCheckpointInterval(10);
			...
			double trainloglikelihood = model.logLikelihood(parsedTrain);
